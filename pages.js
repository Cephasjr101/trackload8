...
(function(){
  ...
  if (truckList || loadList) {
    ...
    var TRUCK_ICON = '<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M1 8h13v8H1z"/><path d="M14 11h4l3 3v2h-7z"/><circle cx="6" cy="18" r="1.8"/><circle cx="17" cy="18" r="1.8"/></svg>';
    var LOAD_ICON = '<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M21 8l-9-5-9 5v8l9 5 9-5z"/><path d="M3.3 8.3L12 13l8.7-4.7M12 13v8"/></svg>';
    function truckCard(t){
      return '<article class="card"><div class="card-head"><span class="card-icon truck">'+TRUCK_ICON+'</span><div class="route"><span>'+esc(t.from_city)+'</span><span class="arrow" aria-hidden="true">→</span><span>'+esc(t.to_city)+'</span></div></div>'+
        '<p class="route-meta">'+esc(t.truck_type)+' · '+Number(t.capacity_kg).toLocaleString()+' kg · '+esc(t.volume_m3)+' m³ · '+esc(t.plate_no)+'</p>'+
        '<p>'+badge(t.status)+'</p>'+
        '<div class="card-foot"><p class="price-tag">'+esc(t.rate_per_km_ghs)+' GH₵/km · returning '+esc(t.available_from)+' → '+esc(t.available_to)+'</p>'+
        '<a class="btn btn-primary btn-sm" href="dashboard.html">Book backhaul</a></div></article>';
    }
    function loadCard(l){
      return '<article class="card"><div class="card-head"><span class="card-icon load">'+LOAD_ICON+'</span><div class="route"><span>'+esc(l.from_city)+'</span><span class="arrow" aria-hidden="true">→</span><span>'+esc(l.to_city)+'</span></div></div>'+
        ...
        '<div class="card-foot"><p class="price-tag">Budget '+fmt(l.budget_ghs)+' · pickup '+esc(l.pickup_date)+'</p>'+
        '<a class="btn btn-ghost btn-sm" href="dashboard.html">Find truck</a></div></article>';
    }
    function fillCities(){
      var sel = $("#mp-city");
      if (!sel) return;
    ...
    function renderLists(){
      ...
      if (truckList){
        ...
        var tCount = $("#truck-count"); if (tCount) tCount.textContent = trucks.length ? "· " + trucks.length + " listed" : "";
      }
      if (loadList){
        ...
        var lCount = $("#load-count"); if (lCount) lCount.textContent = loads.length ? "· " + loads.length + " posted" : "";
      }
    }
  ...
  if (dash) {
    ...
    if (refreshBtn) refreshBtn.addEventListener("click", function(){
      if (API.isLoggedIn()) { initDashboard(); markUpdated(); }
    });
    if (!API.isLoggedIn()) renderLoginGate(); else { initDashboard(); markUpdated(); }
